...
public class License_Plate {
    ...
    public Bitmap get_license_plate(Bitmap input_bitmap, String language) {
...
//            Log.i("车牌处理", "转为Byte  " + "0x"+Integer.toHexString(b));
    ...
    public Bitmap getCenterPicture(Bitmap bitmap) {
        ...
        for (int idx = 0; idx < contours.size(); idx++) {
            ...
            //这个方法的功能是按比例去中间图片  最后的数字是比例值//不是，最后是线的宽度
        ...
        Mat pic = grayMat.submat(100, 250, 183, 640);         //高 宽
...
//        bmp = ContrastPicture(bmp,35);//提高亮度或对比度
//
...
//
        ...
        Imgproc.erode(erodeMat, dilateMat, new Mat());//图片的腐蚀处理
        Imgproc.erode(dilateMat, erodeMat, new Mat());//图片的腐蚀处理
        Imgproc.GaussianBlur(erodeMat, gausMat, new Size(3, 3), 0);//高斯模糊
        Imgproc.dilate(gausMat, dilateMat, new Mat());//图片膨胀处理

//        Imgproc.GaussianBlur(dilateMat, gausMat, new Size(3, 3), 0);//高斯模糊
//        Imgproc.dilate(gausMat, dilateMat, new Mat());//图片膨胀处理
////        Mat return_mat = dilateMat.clone();
////        Core.bitwise_not(dilateMat,return_mat);
////        bmp = Mat2Bitmap(return_mat);
